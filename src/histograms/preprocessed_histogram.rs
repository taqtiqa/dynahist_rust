...
impl BinSketch for BinCopyImpl {
    ...
    fn get_less_count(&self) -> i64 {
        return if self.get_non_empty_index() > 0 {
            self.accumulated_counts[self.get_non_empty_index() - 1]
        } else {
            0
        };
    }
...
impl PreprocessedHistogram {
    ...
    fn new(histogram: impl Histogram) -> Self {
        ...
        if histogram.is_empty() {
            ...
            while !bin_iterator.is_last_non_empty_bin() {
                ...
                accumulated_counts_tmp[non_empty_bin_counter] = accumulated_counts_tmp
                    [non_empty_bin_counter - 1]
                    + bin_iterator.get_bin_count();
                non_empty_bin_counter += 1;
            }
            layout.non_empty_bin_indices =
                Self::copy_of(&non_empty_bin_indices_tmp, non_empty_bin_counter);
            layout.accumulated_counts =
                Self::copy_of(&accumulated_counts_tmp, non_empty_bin_counter);
        }
    }
    ...
    fn get_bin_by_rank(
        &self,
        rank: i64,
    ) -> <crate::histograms::preprocessed_histogram::PreprocessedHistogram as Histogram>::B {
    ...
    fn get_count_of_non_empty_bin(&self, non_empty_bin_index: i32) -> i64 {
        if non_empty_bin_index > 0 {
            return self.accumulated_counts[non_empty_bin_index]
                - self.accumulated_counts[non_empty_bin_index - 1];
        } else {
            return self.accumulated_counts[non_empty_bin_index];
        }
    }
    ...
    fn get_total_count(&self) -> i64 {
        return if self.accumulated_counts.len() > 0 {
            self.accumulated_counts[self.accumulated_counts.len() - 1]
        } else {
            0
        };
    }
    ...
    fn add_values(&self, value: f64, count: i64) -> impl Histogram {
        return Err(DynaHistError::UnsupportedOperationError {
            source: "Not implemented",
        });
    }

    fn add_value(&self, value: f64) -> impl Histogram {
        let io = std::io::Error::new(std::io::ErrorKind::Other, "Not implemented");
        let error = DynaHistError::UnsupportedOperationError { source: io };
        return Err(std::error::Error::source(&error)
            .unwrap()
    ...
    fn write(&self, data_output: &DataOutput) -> Result<(), std::rc::Rc<DynaHistError>> {
        Histogram::create_dynamic(&self.get_layout())
            .add_histogram()
            .write(&data_output)
    }
