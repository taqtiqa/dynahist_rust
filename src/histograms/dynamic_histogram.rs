...
impl AbstractMutableHistogram for DynamicHistogram {
    ...
    fn increase_count(&self,  absolute_index: i32,  count: i64) {
        if absolute_index <= self.get_layout().get_underflow_bin_index() {
            ...
            self.ensure_count_array(absolute_index, absolute_index, &self.determine_required_mode(new_count));
    ...
    fn ensure_count_array(&self,  min_absolute_index: i32,  max_absolute_index: i32,  required_mode: i8) {
        ...
        if self.counts.len() > 0 {
            if min_absolute_index < current_min_absolute_index {
                new_min_absolute_index = std::cmp::max(self.get_layout().get_underflow_bin_index() + 1, &std::cmp::min(min_absolute_index, num::Float::ceil(current_min_absolute_index - current_num_counters * Self::GROW_FACTOR) as i32));
            ...
            if max_absolute_index > current_max_absolute_index {
                new_max_absolute_index = std::cmp::min(self.get_layout().get_overflow_bin_index() - 1, &std::cmp::max(max_absolute_index, num::Float::ceil(current_max_absolute_index + current_num_counters * Self::GROW_FACTOR) as i32));
                is_expansion_necessary = true;
            } else {
    ...
    fn add_histogram_from_estimator(&self,  histogram: impl Histogram,  value_estimator: impl ValueEstimation) -> Self {
                ...
                }
               {
                ...
                loop {
                    ...
                    if merged_count > limit {
                        self.ensure_count_array(&first_bin.get_bin_index(), &first_bin.get_bin_index(), &self.determine_required_mode(merged_count));
