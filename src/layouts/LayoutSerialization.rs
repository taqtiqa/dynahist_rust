...
impl LayoutSerialization {
    ...
    fn new() -> LayoutSerialization {
        ...
        let serial_to_definitions: std::collections::BTreeMap<u64, LayoutSerializationDefinition> =  std::collections::btree_map::BTreeMap::new();

        let class_to_definitions: std::collections::BTreeMap<T, LayoutSerializationDefinition> = std::collections::btree_map::BTreeMap::new();
    }
    ...
    fn register( definitions: &Vec<LayoutSerializationDefinition>)   {
        ...
        let new_serial_to_definitions = std::collections::HashMap::with_capacity(self.serial_to_definitions.size() + definitions.len());

        let new_class_to_definitions = std::collections::HashMap::with_capacity(self.class_to_definitions.size() + definitions.len());

        new_serial_to_definitions.put_all(&serial_to_definitions);
        new_class_to_definitions.put_all(&class_to_definitions);

        for definition in definitions {
            check_argument(! RESERVED_SERIAL_VERSIONS::contains(definition.serialVersion));
             let old_def1: LayoutSerializationDefinition = new_serial_to_definitions.put(definition.serialVersion, definition);
            if old_def1 != null {
                check_argument(&old_def1.clazz.equals(definition.clazz));
            }
             let old_def2: LayoutSerializationDefinition = new_class_to_definitions.put(definition.clazz, definition);
            if old_def2 != null {
    ...
    fn read( data_input: impl DataInput) -> Result<Layout, Rc<DynaHistError>>   {
        ...
        if definition == null {
            return Err(DynaHistError::IOError::new(format!("{} is an unknown layout serialization version!", serialization_version)));
        }
        return Ok(definition.reader.read(&data_input));
    }

    static {
        LayoutSerialization::register( : vec![LayoutSerializationDefinition; 5] = vec![LayoutSerializationDefinition::new(0x7f862c3808df6fcd, CustomLayout.class, CustomLayout::write, CustomLayout::read), LayoutSerializationDefinition::new(0x05d0c7e2dc0316e8, LogLinearLayout.class, LogLinearLayout::write, LogLinearLayout::read), LayoutSerializationDefinition::new(0x9d36115de11d38d6, LogQuadraticLayout.class, LogQuadraticLayout::write, LogQuadraticLayout::read), LayoutSerializationDefinition::new(0x70c0ef16c3809948, LogOptimalLayout.class, LogOptimalLayout::write, LogOptimalLayout::read), LayoutSerializationDefinition::new(0xf6e717a16f0a6a4a, OpenTelemetryExponentialBucketsLayout.class, OpenTelemetryExponentialBucketsLayout::write, OpenTelemetryExponentialBucketsLayout::read), ]
        );
    }
}
