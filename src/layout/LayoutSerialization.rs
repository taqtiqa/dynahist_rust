...
/**
 ...
 let class_to_definitions: Map<Class<?>, LayoutSerializationDefinition> = Collections::empty_map();
...
impl LayoutSerialization {
    ...
    fn  register( definitions: &Vec<LayoutSerializationDefinition>)   {
        if definitions == null {
         ...
         let new_serial_to_definitions: Map<Long, LayoutSerializationDefinition> = HashMap<>::new(serial_to_definitions.size() + definitions.len());
         let new_class_to_definitions: Map<Class<?>, LayoutSerializationDefinition> = HashMap<>::new(class_to_definitions.size() + definitions.len());
        new_serial_to_definitions.put_all(&serial_to_definitions);
        new_class_to_definitions.put_all(&class_to_definitions);
        for  let definition: LayoutSerializationDefinition in definitions {
    ...
    fn  read( data_input: &DataInput) -> /*  throws IOException */Result<Layout, Rc<Exception>>   {
        ...
        if definition == null {
            throw IOException::new(format!("{} is an unknown layout serialization version!", serialization_version));
        }
