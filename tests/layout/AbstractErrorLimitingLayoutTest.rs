...
impl AbstractErrorLimitingLayoutTest {
    ...
    fn test_general(&self) {
        ...
        let relative_bin_width_limits: vec![Vec<f64>; 8] =
            vec![0.0, 1e-3, 1e-2, 1e-1, 1e0, 1e1, 1e2, 1e3];
        ...
        let random: Random = Random::new(0);
        {
            ...
            while i < num_values {
                {
                    values[i] = value_range_lower_bound
                        + random.next_double()
                            * (value_range_upper_bound - value_range_lower_bound);
                }
                i += 1;
            }
        }
        ...
        for absolute_bin_width_limit in absolute_bin_width_limits {
            for relative_bin_width_limit in relative_bin_width_limits {
                let layout: Layout = self.create_layout(
                ...
                for value in values {
                    ...
                    let is_relative_bin_width_limit_fulfilled: bool =
                        Math::abs(upper_bound - lower_bound)
                            / std::cmp::max(&Math::abs(lower_bound), &Math::abs(upper_bound))
                            <= relative_bin_width_limit * (1.0 + eps);
                    let is_absolute_bin_width_limit_fulfilled: bool =
                        Math::abs(upper_bound - lower_bound)
                            <= absolute_bin_width_limit * (1.0 + eps);
                    assert_true(
                        is_absolute_bin_width_limit_fulfilled
                            || is_relative_bin_width_limit_fulfilled,
                    );
                }
                {
                    ...
                    while i <= layout.get_overflow_bin_index() - 1 {
                        {
                            ...
                            let is_relative_bin_width_limit_fulfilled: bool =
                                Math::abs(upper_bound - lower_bound)
                                    / std::cmp::max(
                                        &Math::abs(lower_bound),
                                        &Math::abs(upper_bound),
                                    )
                                    <= relative_bin_width_limit * (1.0 + eps);
                            let is_absolute_bin_width_limit_fulfilled: bool =
                                Math::abs(upper_bound - lower_bound)
            ...
            while bin_index < layout.get_overflow_bin_index() {
                {
                    assert_eq!(
                        bin_index,
                        &layout.map_to_bin_index(&layout.get_bin_lower_bound(bin_index))
                    );
                    assert_eq!(
                        bin_index,
                        &layout.map_to_bin_index(&layout.get_bin_upper_bound(bin_index))
                    );
                }
                bin_index += 1;
            }
    ...
    fn test_get_bin_lower_bound_approximation(&self) {
        ...
        let eps: f64 = 1e-4;
        let layout: GuessLayout =
            self.create_layout(absolute_bin_width_limit, relative_bin_width_limit, 0, 2000);
        {
            ...
            while transition_idx <= layout.get_overflow_bin_index() {
                {
                    ...
                    let transition_low: f64 = std::cmp::min(
                        transition * (1.0 - eps * relative_bin_width_limit),
                        transition - eps * absolute_bin_width_limit,
                    );
                    let transition_high: f64 = std::cmp::max(
                        transition * (1.0 + eps * relative_bin_width_limit),
                        transition + eps * absolute_bin_width_limit,
                    );
    ...
    fn test_create_equidistant_layout(&self) {
        let absolute_error_limits: vec![Vec<f64>; 4] =
            vec![Self::min_normal_f64(), 1.0, 100.0, f64::MAX / i32::MAX];
        for absolute_error_limit in absolute_error_limits {
            // DynaHist::IllegalArgumentError
            assert_eq!(
                self.create_layout(absolute_error_limit, 0, 0, absolute_error_limit * i32::MAX)
                    .err(),
                Some("Illegal argument error")
            );
        }
    }
    ...
    fn test_create(&self) {
        assert_eq!(
            self.create_layout(1e-8, 1e-2, -1e6, f64::INFINITY),
            Some("Illegal argument error")
        );
        assert_eq!(
            self.create_layout(1e-8, 1e-2, f64::NEG_INFINITY, 1e6),
            Some("Illegal argument error")
        );
        assert_eq!(
            self.create_layout(1e-8, 1e-2, 1e6, 1e-6),
            Some("Illegal argument error")
        );
        assert_eq!(
            self.create_layout(1e-8, 1e-2, 1, f64::NAN),
            Some("Illegal argument error")
        );
        assert_eq!(
            self.create_layout(1e-8, 1e-2, f64::NAN, 1),
            Some("Illegal argument error")
        );
        assert_eq!(
            self.create_layout(1e-8, 1e-10, 1e-6, 1e6),
            Some("Illegal argument error")
        );
        assert_eq!(
            self.create_layout(1e-8, 1e-10, i64::MIN, 1e6),
            Some("Illegal argument error")
        );
        assert_eq!(
            self.create_layout(1e-8, 1e-9, 1e-6, 1e6),
            Some("Illegal argument error")
        );
        assert_eq!(
            self.create_layout(Self::min_normal_f64(), 0, 0, f64::MAX),
            Some("Illegal argument error")
        );
        assert_eq!(
            self.create_layout(-1, 1e-2, -1e6, 1e6),
            Some("Illegal argument error")
        );
        assert_eq!(
            self.create_layout(1e-8, -1, -1e6, 1e6),
            Some("Illegal argument error")
        );
        assert_eq!(
            self.create_layout(f64::INFINITY, 1, -1e6, 1e6),
            Some("Illegal argument error")
        );
        assert_eq!(
            self.create_layout(1, f64::INFINITY, -1e6, 1e6),
            Some("Illegal argument error")
        );
        assert_eq!(
            self.create_layout(1, 0, -2, i32::MAX - 3),
            Some("Illegal argument error")
        );
        // no exception should be thrown in this case
        self.create_layout(1, 0, -2, i32::MAX - 4);
        // Should this error - verify against upstream
